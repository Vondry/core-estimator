...
(function (view) {
	...
	function measure(workers, worker_size, sample_size, callback) {
		...
		loop(function (_repeat) {
			let begin, left = worker_size; // Number of workers we are waiting to finish
	...
	function analyse(data) {
		...
		let mean = sum / len;
		let mean_squared = Math.pow(mean, 2);
		let variance = 0;
	...
	function accept(tscore, freedom) {
		...
		const key_low = keys.reduce(function (p, c) {
			if (freedom < c) return p;
			return c;
		});
		const key_high = keys.reduce(function (p, c) {
			if (freedom > c) return p;
			return c;
		});
